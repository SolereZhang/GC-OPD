...
from verl.trainer.ppo.opd_methods import resolve_policy_loss_method
...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _compute_entropy_aware_loss(
        ...
        fire_tensors, metrics = compute_entropy_aware_distill_weights(
            policy_loss_config=plc,
            ref_log_prob=ref_log_prob,
            response_mask=response_mask,
            student_entropys=student_entropys,
            ref_entropys=ref_entropys,
            base_ref_log_prob=base_ref_log_prob,
            base_ref_entropys=base_ref_entropys,
            opd_teacher=opd_teacher,
        )
        token_weight = fire_tensors["token_weight"]
        effective_mask = fire_tensors["effective_mask"]
    ...
    def update_policy(self, data: DataProto):
        ...
        policy_method = resolve_policy_loss_method(self.config.policy_loss)
        gopd_enabled = policy_method == "gopd"
        gc_opd_enabled = policy_method == "gc_opd"
        poweropd_enabled = policy_method == "poweropd"
        uni_opd_enabled = policy_method == "uni_opd_lite"
        if self.config.use_kl_loss or gopd_enabled or policy_method in {
            "opd",
            "gc_opd",
            "poweropd",
            "uni_opd_lite",
        ...
        if uni_opd_enabled:
            select_keys.append("uni_opd_sample_shift")
        ...
        if gopd_enabled and "ep_gopd_lambda" in data.batch.keys():
            select_keys.append("ep_gopd_lambda")
        if gc_opd_enabled:
            for key in ("gc_opd_advantages", "gc_opd_token_weight"):
                if key in data.batch.keys():
                    select_keys.append(key)

        # Include entropy tensors for entropy-aware distillation
        entropy_aware = policy_method == "fire"
        if entropy_aware:
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    # ============================================================
                    # GC-OPD and ExOPD policy-gradient paths
                    # ============================================================
                    if gc_opd_enabled:
                        if "gc_opd_advantages" not in model_inputs:
                            raise KeyError("policy_loss.method=gc_opd requires gc_opd_advantages in actor batch")
                        advantages = model_inputs["gc_opd_advantages"]
                        ...
                        micro_batch_metrics["actor/gc_opd_loss"] = pg_loss.detach().item() * loss_scale_factor
                        valid_gc_opd = response_mask.bool()
                        if "gc_opd_token_weight" in model_inputs and valid_gc_opd.any():
                            token_weight = model_inputs["gc_opd_token_weight"][valid_gc_opd]
                            micro_batch_metrics["actor/gc_opd_token_weight_mean"] = (
                                token_weight.mean().detach().item()
                            )
                        micro_batch_metrics["actor/gc_opd_advantage_abs_mean"] = (
                            advantages[valid_gc_opd].abs().mean().detach().item() if valid_gc_opd.any() else 0.0
                        )

                    elif gopd_enabled:
                        if "ref_log_prob" not in model_inputs:
                            raise KeyError("gopd_enabled=True requires ref_log_prob in actor batch")
                        gopd_tensors, gopd_metrics = compute_gopd_advantages(
                            old_log_prob=old_log_prob,
                            ref_log_prob=model_inputs["ref_log_prob"],
                            response_mask=response_mask,
                            policy_loss_config=self.config.policy_loss,
                            base_ref_log_prob=model_inputs.get("base_ref_log_prob", None),
                            ep_gopd_lambda=model_inputs.get("ep_gopd_lambda", None),
                        )
                        advantages = gopd_tensors["gopd_advantages"]
                        policy_loss_fn = get_policy_loss_fn(loss_mode)
                        pg_loss, pg_metrics = policy_loss_fn(
                            old_log_prob=old_log_prob,
                            log_prob=log_prob,
                        ...
                        micro_batch_metrics.update(pg_metrics)
                        micro_batch_metrics.update(gopd_metrics)
                        micro_batch_metrics["actor/gopd_loss"] = pg_loss.detach().item() * loss_scale_factor

                    # ============================================================
                    # PowerOPD bounded power-transform path
                    # ============================================================
                    elif poweropd_enabled:
                        if "ref_log_prob" not in model_inputs:
                            raise KeyError("policy_loss.method=poweropd requires ref_log_prob in actor batch")
                        power_tensors, power_metrics = compute_poweropd_advantages(
                            old_log_prob=old_log_prob,
                            ref_log_prob=model_inputs["ref_log_prob"],
                            response_mask=response_mask,
                            policy_loss_config=self.config.policy_loss,
                        )
                        advantages = power_tensors["poweropd_advantages"]
                        policy_loss_fn = get_policy_loss_fn(loss_mode)
                        pg_loss, pg_metrics = policy_loss_fn(
                            old_log_prob=old_log_prob,
                            log_prob=log_prob,
                        ...
                        micro_batch_metrics.update(pg_metrics)
                        micro_batch_metrics.update(power_metrics)
                        micro_batch_metrics["actor/poweropd_loss"] = pg_loss.detach().item() * loss_scale_factor

                    # ============================================================
                    # Uni-OPD-lite margin-calibrated OPD path
                    # ============================================================
                    elif uni_opd_enabled:
                        if "ref_log_prob" not in model_inputs:
                            raise KeyError("policy_loss.method=uni_opd_lite requires ref_log_prob in actor batch")
                        if "uni_opd_sample_shift" not in model_inputs:
                            raise KeyError(
                                "policy_loss.method=uni_opd_lite requires precomputed uni_opd_sample_shift"
                            )

                        ref_log_prob = model_inputs["ref_log_prob"].to(device=log_prob.device, dtype=log_prob.dtype)
                        sample_shift = model_inputs["uni_opd_sample_shift"].to(
                            device=log_prob.device, dtype=log_prob.dtype
                        )
                        advantages = (ref_log_prob - old_log_prob + sample_shift[:, None]) * response_mask

                        clip_enabled = _as_bool_config(
                            self.config.policy_loss.get("uni_opd_use_teacher_student_logprob_clip", True)
                        )
                        if clip_enabled:
                            clip_range = float(
                                self.config.policy_loss.get("uni_opd_teacher_student_logprob_clip_range", 10.0)
                            )
                            if clip_range <= 0.0:
                                raise ValueError(
                                    "uni_opd_teacher_student_logprob_clip_range must be > 0 when clip is enabled"
                                )
                            clip_mask = ((log_prob - ref_log_prob).abs() < clip_range).to(dtype=log_prob.dtype)
                            valid_clip_mask = clip_mask * response_mask
                            advantages = advantages * valid_clip_mask
                            clip_keep_ratio = (valid_clip_mask.sum() / response_mask.sum().clamp(min=1)).item()
                        else:
                            clip_range = 0.0
                            clip_keep_ratio = 1.0
                        ...
                        micro_batch_metrics.update(pg_metrics)
                        micro_batch_metrics["actor/uni_opd_loss"] = pg_loss.detach().item() * loss_scale_factor
                        micro_batch_metrics["actor/uni_opd_shift_mean"] = sample_shift.float().mean().item()
                        micro_batch_metrics["actor/uni_opd_shift_abs_mean"] = sample_shift.float().abs().mean().item()
                        micro_batch_metrics["actor/uni_opd_clip_enabled"] = float(clip_enabled)
                        micro_batch_metrics["actor/uni_opd_clip_range"] = clip_range
                        micro_batch_metrics["actor/uni_opd_clip_keep_ratio"] = clip_keep_ratio

                    # ============================================================
                    # Entropy-aware distillation path
                    # ============================================================
                    elif entropy_aware and "ref_entropys" in model_inputs and "student_entropys" in model_inputs:
                        ref_log_prob = model_inputs["ref_log_prob"]
                        ...
                        if policy_method == "opd" and "ref_log_prob" in model_inputs:
                            advantages = -(old_log_prob - model_inputs["ref_log_prob"])
                            opd_adv_clip = float(self.config.policy_loss.get("opd_adv_clip", 0.0))
                            if opd_adv_clip > 0.0:
                                valid_opd = response_mask.bool()
                                if valid_opd.any():
                                    micro_batch_metrics["actor/opd_adv_clip_ratio"] = (
                                        (advantages[valid_opd].abs() > opd_adv_clip)
                                        .float()
                                        .mean()
                                        .detach()
                                        .item()
                                    )
                                advantages = advantages.clamp(min=-opd_adv_clip, max=opd_adv_clip)
