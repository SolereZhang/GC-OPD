...
class PolicyLossConfig(BaseConfig):
    ...
    # Paper method selector. "auto" preserves the upstream OPD/ExOPD/FiRe
    # switches while entry scripts migrate to explicit method names.
    # Active methods: grpo, opd, gopd, gc_opd, fire, poweropd, uni_opd_lite.
    method: str = "auto"
    only_reverse_kl_advantages: bool = False
    multi_teacher_distill: bool = False
    # GOPD/ExOPD reference baseline. When enabled, actor advantages are built as
    # lambda * teacher_log_prob + (1 - lambda) * base_ref_log_prob - old_log_prob.
    gopd_enabled: bool = False
    gopd_lambda: float = 1.0
    gopd_delta_clip: Optional[float] = None
    gopd_position_aware: bool = False
    # Legacy aliases accepted by older OPD/ExOPD entry scripts.
    opd_delta_clip: Optional[float] = None
    position_aware_exopd: bool = False
    # Optional direct-OPD advantage clip. The 8B main-table run used 10.0;
    # the 4B direct-OPD run was unclipped.
    opd_adv_clip: float = 0.0
    # Base-free GC-OPD: A = OPD + beta * credit * residual.
    gc_opd_teacher_score_agg: str = "mean"
    gc_opd_residual_norm: str = "group_zscore"
    gc_opd_residual_beta: float = 0.05
    # uniform, teacher_abs, relative_opd_leverage, relative_teacher_support,
    # or raca.
    gc_opd_credit_mode: str = "relative_opd_leverage"
    gc_opd_credit_cap: float = 5.0
    gc_opd_group_size: int = 8
    gc_opd_min_group_std: float = 1e-6
    gc_opd_min_token_std: float = 1e-6
    gc_opd_adv_clip: float = 10.0
    # PowerOPD bounded power-transform reward. The paper sweeps multiple alpha
    # values; alpha=100 is the local default used for the first matrix baseline.
    poweropd_alpha: float = 100.0
    # Uni-OPD-lite outcome-guided margin calibration. Defaults mirror the
    # public Uni-OPD scripts for the margin/clip component of their recipe.
    uni_opd_use_margin_shift: bool = True
    uni_opd_margin_scope: str = "group"
    uni_opd_margin_mode: str = "mean"
    uni_opd_margin_delta: float = 0.4
    uni_opd_margin_direction: str = "both"
    uni_opd_use_teacher_student_logprob_clip: bool = True
    uni_opd_teacher_student_logprob_clip_range: float = 10.0
