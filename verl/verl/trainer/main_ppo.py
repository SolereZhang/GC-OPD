...
class TaskRunner:
    ...
    def run(self, config):
        ...
        # Same-family OPD can disable this so the teacher scores exactly the
        # same tokenized/truncated context as the student.
        ref_tokenizer = None
        ...
        disable_ref_retokenization = str(
            config.actor_rollout_ref.ref.get(
                "disable_retokenization",
                os.getenv("VERL_FIRE_OPD_DISABLE_REF_RETOKENIZATION", "0"),
            )
        ).lower() in {"1", "true", "yes", "on"}
        if ref_model_path is not None and not disable_ref_retokenization:
            ...
            print("Ref re-tokenization disabled; teacher uses the actor-tokenized same context.")
