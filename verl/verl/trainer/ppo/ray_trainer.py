                ...
                with marked_timer("step", timing_raw):
                    ...
                    if bypass_recomputing_logprobs:  # Use `rollout_log_probs`
                        ...
                        with marked_timer("old_log_prob", timing_raw, color="blue"):
                            ...
                            entropy_aware = resolve_policy_loss_method(
                    ...
                    policy_loss_method = resolve_policy_loss_method(policy_loss_config)
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if policy_loss_method == "uni_opd_lite":
                            if "ref_log_prob" not in batch.batch.keys():
                                raise KeyError("policy_loss.method=uni_opd_lite requires ref_log_prob before advantage")
                            from verl.trainer.ppo.uni_opd import compute_uni_opd_margin_shifts

                            uni_tensors, uni_metrics = compute_uni_opd_margin_shifts(
                            ...
                            for key, value in uni_tensors.items():
                                batch.batch[key] = value
                            metrics.update(uni_metrics)

                        if policy_loss_method == "gc_opd":
                            if "ref_log_prob" not in batch.batch.keys():
                                raise KeyError("policy_loss.method=gc_opd requires ref_log_prob before advantage")
                            from verl.trainer.ppo.gc_opd import add_gc_opd_to_batch

                            batch, gc_opd_metrics = add_gc_opd_to_batch(
                                batch,
                                policy_loss_config=policy_loss_config,
                                rollout_n=self.config.actor_rollout_ref.rollout.n,
                            )
                            metrics.update(gc_opd_metrics)
