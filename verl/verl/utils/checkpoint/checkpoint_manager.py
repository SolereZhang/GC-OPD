...
def _find_latest_ckpt_path_by_scan(path, directory_format):
    if path is None or not os.path.isdir(path):
        return None

    try:
        prefix, suffix = directory_format.split("{}", 1)
    except ValueError:
        return None
    ...
    for dirname in os.listdir(path):
        if not dirname.startswith(prefix) or not dirname.endswith(suffix):
            continue
        step_text_end = len(dirname) - len(suffix) if suffix else len(dirname)
        try:
            iteration = int(dirname[len(prefix) : step_text_end])
        except ValueError:
            continue

        ckpt_path = os.path.join(path, dirname)
        ...
        try:
            ...
            continue
        has_dataloader_state = "data.pt" in ckpt_files or any(
            filename.startswith("data_") and filename.endswith(".pt") for filename in ckpt_files
        )
        if not has_dataloader_state:
            continue
    ...
    if latest_path is not None:
        print(f"Found checkpoint by scanning directories: {latest_path}")
    return latest_path
...
def find_latest_ckpt_path(path, directory_format="global_step_{}"):
    ...
    if not os.path.exists(tracker_file):
        ...
        return _find_latest_ckpt_path_by_scan(path, directory_format)

    try:
        with open(tracker_file, "rb") as f:
            iteration = int(f.read().decode())
    except (OSError, ValueError):
        print(f"Checkpoint tracker file is invalid: {tracker_file}")
        return _find_latest_ckpt_path_by_scan(path, directory_format)
